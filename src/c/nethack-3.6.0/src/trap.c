...
boolean td; /* td == TRUE : trap door or hole */
{
    ...
    if (dont_fall) {
        ...
        impact_drop((struct obj *) 0, currentX(), currentY(), 0);
        if (!td) {
    ...
    if (areYouOnStrongholdLevel()) {
        ...
        int dist = newlevel - currentDungeonLevel();
        dtmp.dnum = currentDungeonNumber();
...
unsigned trflags;
{
    ...
    case ROCKTRAP:
        if (trap->once && trap->tseen && !rn2(15)) {
            ...
            trap->once = 1;
            feeltrap(trap);
            otmp = mksobj_at(ROCK, currentX(), currentY(), TRUE, FALSE);
            otmp->quan = 1L;
            otmp->owt = weight(otmp);

            pline("A trap door in %s opens and %s falls on your %s!",
                  the(ceiling(currentX(), currentY())), an(xname(otmp)), body_part(HEAD));
    ...
    case STATUE_TRAP:
        (void) activate_statue_trap(trap, currentX(), currentY(), FALSE);
        break;
    ...
    case LANDMINE: {
        ...
        if ((youAreLevitating() || youAreFlying()) && !forcetrap) {
            ...
            saddle = sobj_at(SADDLE, currentX(), currentY());
        ...
        newsym(currentX(), currentY()); /* update trap symbol */
        losehp(Maybe_Half_Phys(rnd(16)), "land mine", KILLED_BY_AN);
        /* fall recursively into the pit... */
        if ((trap = t_at(currentX(), currentY())) != 0)
...
register struct monst *mtmp;
{
    ...
    if (!trap) {
        ...
        } else if (isMetallivorous(mptr->monsterTypeID)) {
        ...
        if (!inescapable && ((mtmp->mtrapseen & (1 << (tt - 1))) != 0
                             || (tt == HOLE && !isMindless(mptr->monsterTypeID)))) {
	...
	int pmid = mptr->monsterTypeID;
        ...
        case SQKY_BOARD:
            ...
            break;
        case BEAR_TRAP:
            if (monsterSize(pmid) > MZ_SMALL && !isAmorphous(pmid) && !isFlyer(pmid)
            ...
            break;
        case SLP_GAS_TRAP:
            if (!resists_sleep(mtmp) && !doesNotBreathe(mptr->monsterTypeID) && !mtmp->msleeping
                && mtmp->mcanmove) {
        ...
        case SPIKED_PIT:
            ...
            if (!passesThroughWalls(mptr->monsterTypeID))
                mtmp->mtrapped = 1;
            if (in_sight) {
        ...
        /* inWater() msgs already in spoteffects()/drown() */
            ...
            if (!(emask & W_SADDLE)) {
                if (Sokoban && trap) {
                    ...
                    You("float gently to the %s.", surface(currentX(), currentY()));
                }
            }
        }
    }
...
struct obj *box; /* null for floor trap */
{
    ...
    if (!box && burn_floor_objects(currentX(), currentY(), see_it, TRUE) && !see_it)
        You("smell paper burning.");
    if (is_ice(currentX(), currentY()))
        melt_ice(currentX(), currentY(), (char *) 0);
}
            ...
            for (i = -1; i <= 1; i++)
                for (j = -1; j <= 1; j++) {
                    if (!isok(currentX() + i, currentY() + j))
                        continue;
                    mtmp = m_at(currentX() + i, currentY() + j);
                    if (mtmp)
...
crawl:
    ...
    for (i = 0; i < 5; i++) { /* arbitrary number of loops */
        ...
        pool_of_water = waterbody_name(currentX(), currentY());
...
boolean force_failure;
{
    ...
    int ttype = ttmp->ttyp;
    boolean under_u = (!directionX() && !directionY());
...
boolean force;
{
    ...
    here = (x == currentX() && y == currentY()); /* !directionX && !directionY() */
...
{                 /* otherwise left with its previous value intact */
    ...
    if (mon == u.usteed)
        ishero = TRUE;
    t = t_at(ishero ? currentX() : mon->mx, ishero ? currentY() : mon->my);
    ...
    if (currentLuckWithBonus() > -13 && rn2(13 + currentLuckWithBonus()) > 7) { /* saved by luck */
        ...
        case 21: {
            ...
            insider = (youAreCurrentlyOccupyingAShop() && inside_shop(currentX(), currentY())
                       && shopLocatedAt(ox, oy) == mostRecentCurrentlyOccupiedShopID());
