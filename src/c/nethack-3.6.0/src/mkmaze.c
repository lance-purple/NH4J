...
fixup_special()
{
    ...
    if (areYouOnMedusaLevel()) {
        ...
        if (rn2(2))
            ...
            otmp =
                makeCorpseOrStatue(STATUE, (struct monst *) 0, NON_PM,
...
#undef y_maze_min
    ...
    for (x = rn1(5, 7); x; x--) {
        mazexy(&mm);
        (void) makeMonsterOfType(NON_PM, mm.x, mm.y, NO_MM_FLAGS);
    }
    for (x = rn1(6, 7); x; x--) {
        mazexy(&mm);
...
water_friction()
{
    ...
    if (directionX() && !rn2(!directionY() ? 3 : 6)) { /* 1/3 chance or half that */
        ...
        setDirectionY(dy);
        eff = TRUE;
    } else if (directionY() && !rn2(!directionX() ? 3 : 5)) { /* 1/3 or 1/5*(5/6) */
