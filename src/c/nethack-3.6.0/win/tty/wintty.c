...
int race, gend, algn; /* all roleNone() for !filtering case */
{
    anything any;
    boolean role_ok;
    char thisch, lastch = '\0', maleRoleNameBuff[50], fullRoleNameBuff[50];
	...
	releaseJavaString(maleRoleName);
        ...
        if (filtering)
            ...
            any.a_string = maleRoleNameBuff;
	}

        thisch = lowc(*maleRoleNameBuff);
        if (thisch == lastch) {
            thisch = highc(thisch);
	}
	    ...
	    javaString femaleRoleName = roleNameAsMale(i);
            ...
            if (gend == 1) {
                ...
                Strcpy(fullRoleNameBuff, femaleRoleName.c_str);
            } else if (gend < 0) {
                /* not chosen yet; append slash+female name */
                Strcat(fullRoleNameBuff, "/");
                Strcpy(fullRoleNameBuff, femaleRoleName.c_str);
            }
        ...
        add_menu(win, NO_GLYPH, &any, thisch, 0, ATR_NONE, an(fullRoleNameBuff),
...
register int xmin, ymax;
{
    ...
    if (swallowed()) { /* Can be done more efficiently */
...
int x, y;
{
    ...
    if (clipx != oldx || clipy != oldy) {
        if (onSameLevelAsLastTurn() && !restoring)
            (void) doredraw();
    }
}
#endif /* CLIPPING */
