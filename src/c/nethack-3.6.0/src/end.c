...
int how;
{
    ...
    endtime = getnow();
    setElapsedPlayingTimeSeconds(elapsedPlayingTimeSeconds() + (long) (endtime - epochSecondsOfMostRecentRestore()));
    ...
    if (how == ESCAPED || how == ASCENDED) {
        ...
        if (Schroedingers_cat) {
            int mhp;
            int m_lev = adjustMonsterLevel(PM_HOUSECAT);
            mhp = d(m_lev, 8);
        ...
        if (!done_stopprint) {
            Sprintf(eos(pbuf), "%s with %ld point%s,",
                    ...
                    currentScore(), plur(currentScore()));
            putstr(endwin, 0, pbuf);
        }
        ...
        Sprintf(eos(pbuf), " with %ld point%s,", currentScore(), plur(currentScore()));
        putstr(endwin, 0, pbuf);
    }
