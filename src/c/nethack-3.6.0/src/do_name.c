...
namefloorobj()
{
    ...
    Sprintf(buf, "object on map (or '.' for one %s you)",
            (lurking() && hidesUnderStuff(pmid4you())) ? "over" : "under");
	...
	switch (rn2(6)) {
            /* straight role name */
            case 0:  funnyName = (currentlyFemale && roleNameHasFemaleVersion(yourCurrentRoleID()))
...
boolean called;
{
    ...
    do_it = !canspotmon(mtmp) && article != ARTICLE_YOUR
            ...
            && !(swallowed() && mtmp == u.ustuck) && !(suppress & SUPPRESS_IT);
