...
boolean at_stairs, falling, portal;
{
    ...
    if (areYouInHell() && up && haveSpecialItem(SPECIAL_ITEM_AMULET) && !newdungeon && !portal
        ...
        if (!rn2(4)) {
            int odds = 3 + (int) currentAlignmentType(),   /* 2..4 */
     ...
     */
    if (areYouOnLevel(&qstart_level) && !newdungeon && !ok_to_quest()) {
    ...
    resetMonsterRandomizer(NON_PM); /* currentLevel change affects monster generation */
...
const char *pre_msg, *post_msg;
{
    ...
    setTypeOfLevelYouWereSentTo(typmask);
    /* destination level */
...
deferred_goto()
{
    if (!areYouBeingSentToSameLevel()) {
        d_level dest;
        int typmask = typeOfLevelYouWereSentTo(); /* save it; goto_level zeroes typeOfLevelYouWereSentTo */
    ...
    setTypeOfLevelYouWereSentTo(0); /* our caller keys off of this */
    if (dfr_pre_msg)
