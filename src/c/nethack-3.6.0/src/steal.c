...
register struct monst *mtmp;
{
    ...
    if (fgold && (!ygold || fgold->quan > ygold->quan || !rn2(5))) {
        ...
        pline("%s quickly snatches some gold from %s %s %s!", Monnam(mtmp),
              (youAreLevitating() || youAreFlying()) ? "beneath" : "between", whose, what);
        if (!ygold || !rn2(5)) {
        ...
        if (otmp->o_id == stealoid) {
            for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
                if (mtmp->m_id == stealmid) {
                    ...
                    if (!monsterTypeCanCauseDamageType(pmid4mon(mtmp), AD_SITM)) /* polymorphed */
                        goto botm;
                    if (otmp->unpaid)
...
gotobj:
    ...
    could_petrify =
        (otmp->otyp == CORPSE && touchPetrifies(otmp->corpsenm));
    (void) mpickobj(mtmp, otmp); /* may free otmp */
...
int ochance, achance; /* percent chance for ordinary item, artifact */
{
    ...
    if (carried(obj)) {
        ...
        if (obj->unpaid)
            subfrombill(obj, shop_keeper(mostRecentCurrentlyOccupiedShopID()));
        ...
        if (mon->mhp > 0) {
            ...
            /* don't charge for an owned saddle on dead steed (provided
               that the hero is within the same shop at the time) */
                   ...
                   && oneOfRoomsHasID(allShopsLocatedAt(currentX(), currentY()),
                            levl[omx][omy].roomno)) {
            obj->no_charge = 1;
        }
