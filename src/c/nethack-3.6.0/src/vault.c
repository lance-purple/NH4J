...
boolean forceshow;
{
    ...
    while ((fcbeg = egrd->fcbeg) < egrd->fcend) {
        ...
        if ((grd->mhp <= 0 || !in_fcorridor(grd, currentX(), currentY())) && egrd->gddone)
            forceshow = TRUE;
        ...
        if ((mtmp = m_at(fcx, fcy)) != 0) {
            if (mtmp->isgd) {
                return FALSE;
            } else if (!in_fcorridor(grd, currentX(), currentY())) {
                if (mtmp->mtame)
                    yelp(mtmp);
    ...
    if (IS_ROCK(levl[currentX()][currentY()].typ))
...
char
mostRecentCurrentlyOccupiedVaultID()
{
    register char *ptr;
...
invault()
{
    ...
    struct monst *guard;
    boolean gsensed;
    int trycount, vaultroom = (int) mostRecentCurrentlyOccupiedVaultID();
    ...
    if (timeInVault() % 30 == 0 && !guard) { /* if time ok and no guard now. */
        ...
        for (dd = 2; (dd < ROWNO || dd < COLNO); dd++) {
            for (y = currentY() - dd; y <= currentY() + dd; ly = y, y++) {
                if (y < 0 || y > ROWNO - 1)
                    continue;
                for (x = currentX() - dd; x <= currentX() + dd; lx = x, x++) {
                    if (y != currentY() - dd && y != currentY() + dd && x != currentX() - dd)
                        x = currentX() + dd;
                    if (x < 1 || x > COLNO - 1)
                        continue;
    ...
    fnd:
        ...
        if (youAreBeingStrangled() || isSilent(youmonst.data->monsterTypeID) || multi < 0) {
...
register struct monst *grd;
{
    ...
    boolean u_in_vault = ('\0' != mostRecentCurrentlyOccupiedVaultID()) ? TRUE : FALSE;
    ...
    if (!grd->mpeaceful) {
        ...
        if (!u_in_vault
            && (grd_in_vault || (in_fcorridor(grd, grd->mx, grd->my)
                                 && !in_fcorridor(grd, currentX(), currentY())))) {
            (void) rloc(grd, FALSE);
            wallify_vault(grd);
    ...
    if (egrd->fcend > 1) {
        if (egrd->fcend > 2 && in_fcorridor(grd, grd->mx, grd->my)
            && !egrd->gddone && !in_fcorridor(grd, currentX(), currentY())
        ...
        if (u_carry_gold && (in_fcorridor(grd, currentX(), currentY())
                             /* cover a 'blind' spot */
...
newpos:
    ...
    cleanup:
        ...
        egrd->ogy = grd->my;
        restfakecorr(grd);
        if (!semi_dead && (in_fcorridor(grd, currentX(), currentY()) || cansee(x, y))) {
...
gd_sound()
{
    ...
    if ('\0' != mostRecentCurrentlyOccupiedVaultID())
        return FALSE;
    else
