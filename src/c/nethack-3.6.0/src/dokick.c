...
xchar x, y;
{
    ...
    if (!uarmf && kickedobj->otyp == CORPSE
        && touchPetrifies(kickedobj->corpsenm) && !youResistStoning()) {
    ...
    if (!ZAP_POS(levl[x + directionX()][y + directionY()].typ)
        || closed_door(x + directionX(), y + directionY()))
        range = 1;
    ...
    newsym(x, y);
    mon = bhit(directionX(), directionY(), range, KICKED_WEAPON,
...
dokick()
{
    ...
    if (mtmp) {
        ...
        if ((areYouOnAirLevel() || youAreLevitating()) && context.move) {
            ...
            if (range < 1)
                range = 1;
            hurtle(-directionX(), -directionY(), range, TRUE);
        }
        return 1;
    }
    if (glyph_is_invisible(levl[x][y].glyph)) {
            ...
            if (rn2(15) && !(maploc->looted & TREE_LOOTED)
                ...
                while (cnt--) {
                    if (canPlaceMonsterNear(&mm, mm.x, mm.y, PM_KILLER_BEE, 0)
                        && makeMonsterOfType(PM_KILLER_BEE, mm.x, mm.y, MM_ANGRY))
                        made++;
                }
                if (made)
...
schar loc;
{
    ...
    case MIGR_RANDOM: /* trap door or hole */
        if (areYouOnStrongholdLevel()) {
            ...
            break;
        } else if (areYouInEndgame() || areYouOnBottomLevel()) {
            cc->y = cc->x = 0;
            break;
...
xchar x, y;
{
    ...
    if (areYouOnLevel(&qstart_level) && !ok_to_quest())
        return MIGR_NOWHERE;
