...
static boolean monsterInWater(int pmid) {
...
struct obj *otmp;
{
    ...
    case SPE_STONE_TO_FLESH:
        if (pmid4mon(mtmp) == PM_STONE_GOLEM) {
    ...
    if (wake) {
        if (mtmp->mhp > 0) {
            wakeup(mtmp);
            m_respond(mtmp);
            if (mtmp->isshk && youAreNotCurrentlyOccupyingAShop())
                hot_pursuit(mtmp);
...
coord *cc;
{
    ...
    if (mtmp2) {
        ...
        /* heal the monster */
        if (mtmp->mhpmax > mtmp2->mhpmax && isRiderOfTheApocalypse(pmid4mon(mtmp2)))
...
boolean by_hero;
{
    ...
    if (MON_AT(x, y)) {
        if (canPlaceMonsterNear(&xy, x, y, montype, 0))
            x = xy.x, y = xy.y;
    }
    ...
    case OBJ_INVENT:
        ...
        break;
    case OBJ_FLOOR:
        /* in case MON_AT+canPlaceMonsterNear for invisible mon */
        x = corpse->ox, y = corpse->oy;
...
int ochance, achance; /* percent chance for ordinary objects, artifacts */
{
    if (obj->otyp == AMULET_OF_YENDOR
        ...
        || (obj->otyp == CORPSE && isRiderOfTheApocalypse(obj->corpsenm))) {
        return TRUE;
    } else {
        int chance = rn2(100);
                    ...
                    * if object is destroyed */
{
    ...
    while (range-- > 0) {
        ...
        if (in_skip) {
            if (range <= skiprange_end) {
                ...
                if (range > 3) /* another bounce? */
                    skiprange(range, &skiprange_start, &skiprange_end);
            } else if (mtmp && monsterInWater(pmid4mon(mtmp))) {
        ...
        if (mtmp && !(in_skip && monsterInWater(pmid4mon(mtmp)))) {
        ...
        make_bounce:
            ...
            if (range && isok(lsx, lsy) && cansee(lsx, lsy)) {
                    pline("%s %s!", The(fltxt),
                          ...
                          : "bounces");
                    if (areYouOnAirLevel()) goto get_out_buzz;
            }
            if (!dx || !dy || !rn2(20)) {
                dx = -dx;
                dy = -dy;
    ...
    if ((mon = m_at(x, y)) != 0) {
        ...
        if (type >= 0) {
            ...
            if (mon->ispriest && locationIsInATemple(mon->mx, mon->my))
                ghod_hitsu(mon);
            if (mon->isshk && youAreNotCurrentlyOccupyingAShop())
                hot_pursuit(mon);
        }
    }
    return rangemod;
...
int damage, tell;
{
    ...
    if (dlev > 50)
        ...
        dlev = isMonsterPlayer(pmid4mon(mtmp)) ? currentExperienceLevel() : 1;
