...
pet_type()
{
    if (yourRoleHasAPreferredPet())
    {
        return pmidOfPreferredPetForYourRole();
    }
...
boolean quietly;
{
    ...
    if (mtmp->mtame && monsterHasAttackType(pmid4mon(mtmp), AT_WEAP)) {
...
losedogs()
{
    ...
    for (mtmp = migrating_mons; mtmp; mtmp = mtmp->nmon) {
        if (mtmp->mux != currentDungeonNumber() || mtmp->muy != currentDungeonLevel())
            continue;
        if (mtmp->isshk) {
