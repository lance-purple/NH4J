    ...
    case DUNCE_CAP:
        ...
        if (youAreHallucinating()) {
            ...
            You_feel("%s.", /* track INT change; ignore WIS */
                     ACURR(A_INT)
                             <= (yourCurrentAttr(A_INT) + yourAttrBonus(A_INT) + yourTemporaryAttrChange(A_INT))
...
boolean voluntary; /* taking gloves off on purpose? */
{
    ...
    if (touchPetrifies(obj->corpsenm) && !youResistStoning()) {
...
{
    boolean already_blind = youCannotSee(), changed = FALSE;
    ...
    if (youCannotSee() && !already_blind) {
        ...
        if (youAreBeingPunished())
            set_bc(0);
    } else if (already_blind && youCanSee()) {
        changed = TRUE;
...
register struct obj *otmp;
{
    boolean was_blind = youCannotSee(), changed = FALSE;
