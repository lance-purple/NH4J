...
bot1()
{
    ...
    Sprintf(nb = eos(nb),
            (currentAlignmentType() == A_CHAOTIC)
                ? "  Chaotic"
                : (currentAlignmentType() == A_NEUTRAL) ? "  Neutral" : "  Lawful");
#ifdef SCORE_ON_BOTL
    if (flags.showscore)
...
bot2()
{
    ...
    hp = areYouPolymorphed() ? currentHitPointsAsMonster() : currentHitPoints();
    hpmax = areYouPolymorphed() ? maximumHitPointsAsMonster() : maximumHitPoints();
...
botl_score()
{
    ...
    if ((utotal -= startingMoney()) < 0L)
        utotal = 0L;
    utotal += currentScore() + (50 * (deepest - 1))
          + (deepest > 30 ? 10000 : deepest > 20 ? 1000 * (deepest - 20) : 0);
    if (utotal < currentScore())
...
bot()
{
    ...
    blstats[idx][BL_HP].a.a_int = areYouPolymorphed() ? currentHitPointsAsMonster() : currentHitPoints();
    blstats[idx][BL_HPMAX].a.a_int = areYouPolymorphed() ? maximumHitPointsAsMonster() : maximumHitPoints();
    ...
    blstats[idx][BL_AC].a.a_int = armorClass();
