...
unsigned int *stuckid, *steedid;
{
    ...
    for (int i =  0; i < maximumOccupiedRoomCount(); i++) {
        setCurrentlyOccupiedRoomIDs(i, read_char((fd)));
    ...
    int count = read_int(fd);
    for (int i = 0; i < count; i++) {
        addWeaponSkillAdvance(read_int(fd));
    }
...
register int fd;
{
    ...
    You("return to level %d in %s%s.", currentDepth(),
                ...
                mtmp->mpeaceful =
                    (isUnicorn(pmid4mon(mtmp))
                     && sgn(currentAlignmentType()) == sgn(monsterAlignment(pmid4mon(mtmp))))
                        ? TRUE
                        : peacefullyMinded(pmid4mon(mtmp));
            set_malign(mtmp);
        } else if (elapsed > 0L) {
