        ...
        if (distanceSquaredToYou(mtmp->mx, mtmp->my)
            ...
            if (!um_dist(mtmp->mx, mtmp->my, 3)) {
                ...
                if (um_dist(mtmp->mx, mtmp->my, 5)
                    ...
                    if (mtmp->mhp > 0)
                        setPacifistConduct(save_pacifism);
                } else {
...
struct obj *obj;
{
    ...
    if (touchPetrifies(corpse->corpsenm) && !youResistStoning()
        && !uarmg) {
        char kbuf[BUFSZ];
...
long timeout;
{
    ...
    if (figurine->where == OBJ_INVENT || figurine->where == OBJ_MINVENT)
        okay_spot = canPlaceMonsterNear(&cc, cc.x, cc.y, figurine->corpsenm, 0);
        ...
        if (mtmp->mundetected) {
            if (hidesUnderStuff(pmid) && mshelter) {
                Sprintf(and_vanish, " and %s under %s",
                        locomotionVerb(pmid4mon(mtmp), "crawl"), doname(mshelter));
        ...
        case OBJ_INVENT:
            if (youCannotSee() || suppress_see)
                You_feel("%s %s from your pack!", something,
                         locomotionVerb(pmid4mon(mtmp), "drop"));
            else
                You_see("%s %s out of your pack%s!", monnambuf,
                        locomotionVerb(pmid4mon(mtmp), "drop"), and_vanish);
            break;
        ...
        case OBJ_MINVENT:
            if (cansee_spot && !silent && !suppress_see) {
                ...
                You_see("%s %s out of %s%s!", monnambuf,
                        locomotionVerb(pmid4mon(mtmp), "drop"), carriedby,
                        and_vanish);
            }
            break;
#if 0
    ...
    if (swallowed() && attack(u.ustuck)) {
        ...
        if (wrapped_what) {
            ...
            if (proficient && rn2(proficient + 2)) {
                if (!mtmp || canPlaceMonsterNear(&cc, rx, ry, pmid4you(), 0)) {
...
struct obj *obj;
{
    ...
    case 2: /* Monster */
        ...
        if (isVerySmallMonster(pmid4mon(mtmp)) && !rn2(4)
            && canPlaceMonsterNear(&cc, currentX(), currentY(), NON_PM, 0)) {
            flags.confirm = FALSE;
