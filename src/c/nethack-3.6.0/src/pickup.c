...
boolean remotely;
{
    if (!obj || obj->otyp != CORPSE || !isRiderOfTheApocalypse(obj->corpsenm))
        return FALSE;
...
struct obj *otmp;
{
    ...
    if (!swallowed() && otmp != uball && costly_spot(otmp->ox, otmp->oy)) {
        ...
        savedShopIDs[0] = currentlyOccupiedShopIDs(0);
        savedShopIDs[1] = currentlyOccupiedShopIDs(1);

        fakeShopID = shopLocatedAt(otmp->ox, otmp->oy);

        setCurrentlyOccupiedShopIDs(0, fakeShopID);
        setCurrentlyOccupiedShopIDs(1, '\0');

        /* sets obj->unpaid if necessary */
        addtobill(otmp, TRUE, FALSE, FALSE);

        setCurrentlyOccupiedShopIDs(0, savedShopIDs[0]);
        setCurrentlyOccupiedShopIDs(1, savedShopIDs[1]);

        /* if you're outside the shop, make shk notice */
        if (noneOfCurrentlyOccupiedShopsHasID(fakeShopID)) {
    ...
    if (!IS_THRONE(levl[x][y].typ)) {
        ...
        for (coffers = fobj; coffers; coffers = coffers->nobj)
            if (coffers->otyp == CHEST) {
                ...
                if (!otmp
                    || distanceSquaredToYou(coffers->ox, coffers->oy)
                           < distanceSquaredToYou(otmp->ox, otmp->oy))
        ...
        if (coffers) {
            ...
            if (!coffers->olocked) {
                ...
                (void) boxlock(coffers, &boxdummy);
                   ...
                   (mon = makeMonsterOfType(throneRoomMonsterType(), x, y, NO_MM_FLAGS)) != 0) {
            freeinv(goldob);
...
struct obj *item;
{
    ...
    if (youAreCurrentlyOccupyingAShop() && (shkp = shop_keeper(mostRecentCurrentlyOccupiedShopID())) != 0) {
...
struct obj *box;
{
    ...
    if (livecat) {
        ...
        deadcat =
            makeNamedObject(CORPSE, PM_HOUSECAT, box->ox, box->oy, sc);
        if (deadcat) {
            obj_extract_self(deadcat);
            (void) add_to_container(box, deadcat);
        }
        pline_The("%s inside the box is dead!",
                  youAreHallucinating() ? rndmonnam(NULL) : "housecat");
    }
    box->owt = weight(box);
    return;
}
